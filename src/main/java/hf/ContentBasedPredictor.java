...
public class ContentBasedPredictor extends Predictor {
    ...
    public void createAllUserMetaWordProfiles(boolean saveProfilesToFile) {
        userProfiles = new TIntObjectHashMap<TObjectDoubleHashMap<String>>(dbExt.numUsers());
        int nUsers = 0;
        int percent = 1;
    ...
    public TObjectDoubleHashMap<String> createUserMetaWordProfile(int uIdx) {
        ...
        int numItems = 0;
        TObjectDoubleHashMap<String> wordCounts = new TObjectDoubleHashMap<String>(100);
        TObjectDoubleHashMap<String> wordItemAppearences = new TObjectDoubleHashMap<String>(100);

        for (Database.Event e : db.userEvents(uIdx, null)) {
            numItems++;
            HashSet<String> appearedWordsInItem = new HashSet<String>(100);
            for (String key : keys) {
